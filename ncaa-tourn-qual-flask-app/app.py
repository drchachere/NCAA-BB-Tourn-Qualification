...
def index():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                df_scaled = scaler.transform(user_df.iloc[:, 0:24])
                data_scaled = pd.DataFrame(df_scaled, columns=['G', 'W', 'L', 'W-L%', 'SRS', 'SOS', 'W.1', 'L.1', 'W.2', 'L.2', 'W.3', 'L.3', 'Tm.', 'Opp.', 'FTr', '3PAr', 'TS%', 'TRB%', 'AST%', 'BLK%', 'eFG%', 'TOV%', 'FT/FGA', 'Conf_Rk'])
                ...
                data_ml = pd.concat([data_scaled, data_dum], axis=1)
